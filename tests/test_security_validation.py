...
class TestDataEncryption:
    ...
    def test_admin_tls_endpoints(self):
        """Smoke-test TLS status and self-signed generation endpoints."""
        from fastapi.testclient import TestClient
        from core.main import app
        ...
        r1 = client.get("/admin/security/tls/status", headers=headers)
        assert r1.status_code == 200
        payload = r1.json()
        assert "min_version" in payload

        r2 = client.post(
            "/admin/security/tls/self-signed",
        ...
        assert js.get("cert_path") and js.get("key_path")
...
class TestPluginSecurity:
    ...
    def test_plugin_manifest_validation(self):
        ...
        valid_manifest = PluginManifest(
            ...
            traits=["handles_pii"],
            security={"authentication_required": True, "data_classification": ["pii"]},
            compliance={"hipaa_controls": ["164.312(a)"], "audit_level": "detailed"},
        )
    ...
    def test_plugin_domain_allowlist(self):
        ...
        # Test unsafe domains
        unsafe_domains = ["localhost", "127.0.0.1", "10.0.0.1", "192.168.1.1"]
