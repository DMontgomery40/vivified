...
from .models import Message, Event, MessageFilter, MessageDeliveryStatus
...
class MessagingService:
    ...
    async def publish_event(
        ...
        if success:
            ...
            raise PermissionError(f"Plugin {source_plugin} not authorized to publish event {event_type}")
    ...
    async def send_message(
        ...
        if success:
            ...
            raise PermissionError(f"Plugin {source_plugin} not authorized to send message to {target_plugin}")
    ...
    async def get_messages(
        ...
        if filter_criteria.source_plugins:
            messages = [m for m in messages if m.source_plugin in filter_criteria.source_plugins]
        
        if filter_criteria.target_plugins:
            messages = [m for m in messages if m.target_plugin in filter_criteria.target_plugins]
        
        if filter_criteria.data_classifications:
            messages = [m for m in messages if m.data_classification in filter_criteria.data_classifications]
        
        if filter_criteria.traits:
            messages = [m for m in messages if any(trait in m.traits for trait in filter_criteria.traits)]
        ...
        if filter_criteria.created_after:
            messages = [m for m in messages if m.created_at >= filter_criteria.created_after]
        
        if filter_criteria.created_before:
            messages = [m for m in messages if m.created_at <= filter_criteria.created_before]
        
        if filter_criteria.correlation_id:
            messages = [m for m in messages if m.correlation_id == filter_criteria.correlation_id]
    ...
    async def cleanup_expired_messages(self):
        ...
        now = datetime.utcnow()
        expired_messages = [
            msg_id for msg_id, message in self.message_store.items()
