...
export default function Plugins({ client, readOnly = false }: Props) {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError('');
      // Check UI config to see if feature is enabled; fallback to true
      try {
        const uic = await client.getUiConfig();
        setPluginsEnabled(Boolean(uic?.features?.plugins?.enabled ?? true));
      } catch {
