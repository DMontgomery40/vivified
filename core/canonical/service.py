...
class CanonicalService:
    ...
    async def normalize_user(
        ...
        try:
            ...
            if not await self._can_transform_data(source_plugin, target_plugin, "user"):
                raise PermissionError(f"Plugin {source_plugin} not authorized to transform user data for {target_plugin}")
    ...
    async def normalize_message(
        ...
        try:
            # Check if transformation is allowed
            if not await self._can_transform_data(source_plugin, target_plugin, "message"):
                raise PermissionError(f"Plugin {source_plugin} not authorized to transform message data for {target_plugin}")
            ...
            await self.audit_service.log_event(
                ...
                plugin_id=source_plugin,
                level=AuditLevel.STANDARD,
                details={"message_id": canonical_message.id, "target_plugin": target_plugin}
            )
    ...
    async def normalize_event(
        ...
        try:
            # Check if transformation is allowed
            if not await self._can_transform_data(source_plugin, target_plugin, "event"):
                raise PermissionError(f"Plugin {source_plugin} not authorized to transform event data for {target_plugin}")
            ...
            await self.audit_service.log_event(
                ...
                plugin_id=source_plugin,
                level=AuditLevel.STANDARD,
                details={"event_id": canonical_event.event_id, "target_plugin": target_plugin}
            )
    ...
    async def denormalize_data(
        ...
        try:
            # Check if transformation is allowed
            if not await self._can_transform_data(source_plugin, target_plugin, "denormalize"):
                raise PermissionError(f"Plugin {source_plugin} not authorized to denormalize data for {target_plugin}")
            ...
            await self.audit_service.log_event(
                ...
                plugin_id=source_plugin,
                level=AuditLevel.STANDARD,
                details={"target_format": target_format, "target_plugin": target_plugin}
            )
    ...
    async def list_canonical_data(
        ...
        if data_type:
            items = [item for item in items if f"{data_type}:" in str(type(item).__name__).lower()]
