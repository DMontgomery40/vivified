...
from .models import ProxyRequest, ProxyResponse, ProxyStatus, RateLimit
...
class ProxyHandler:
    ...
    async def _is_request_allowed(
        ...
        if allowed_paths and not any(request.url.path.startswith(path) for path in allowed_paths):
            return False
