...
class TraitRegistry:
    ...
    def _load_defaults(self) -> None:
        ...
        # Register all traits
        all_traits = (
            role_traits + capability_traits + data_access_traits + 
            ui_feature_traits + plugin_type_traits + security_traits + 
            compliance_traits + system_traits + legacy_traits
        )
    ...
    def get_traits_by_category(self, category: TraitCategory) -> List[Trait]:
        ...
        return [self._traits[name] for name in self._categories[category] if name in self._traits]
    ...
    def validate_trait_combination(self, traits: List[str]) -> Tuple[bool, List[str]]:
        ...
        for trait_name in traits:
            ...
            if trait.is_legacy:
                logger.warning(f"Using legacy trait: {trait_name}, consider using: {trait.aliases}")
...
class TraitValidator:
    ...
    def validate_user_traits(self, traits: List[str], user_context: Dict[str, Any] = None) -> Tuple[bool, List[str]]:
        ...
        if user_context:
            ...
            sensitive_traits = [t for t in traits if self.registry.get_trait(t) and self.registry.get_trait(t).is_sensitive]
            if sensitive_traits and not user_context.get("admin_approved", False):
                errors.append(f"Sensitive traits require admin approval: {sensitive_traits}")
        
        return len(errors) == 0, errors

    def validate_plugin_traits(self, traits: List[str], plugin_context: Dict[str, Any] = None) -> Tuple[bool, List[str]]:
        ...
        if plugin_context:
            ...
            if plugin_type:
                ...
                type_traits = [t for t in traits if self.registry.get_trait(t) and 
                             self.registry.get_trait(t).category == TraitCategory.PLUGIN_TYPE]
                if not type_traits:
                    errors.append(f"Plugin must have a type trait (communication_plugin, storage_plugin, etc.)")
        
        return len(errors) == 0, errors

    def get_required_traits_for_data_classification(self, classification: str) -> List[str]:
